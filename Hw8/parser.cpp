...
//
// Description: Parser Class, finds 4 types for id from .log file, 
//						State machine, point extraction from each NMEA sentence found in log file 
//						and function prototypes definitions used in main. 
//
// Input: none
//
// Output: none
...
//This will parse The GPGGA Sentence specifically 
...
//mode 
	...
	//pointers to the struct variables holding data 
	point->UTC_time=UTC_time; //pointer to UTC Time 
	point->lat_first_A=lat_first; // pointer to the first part of latitude winch is the day 
	point->lat_last_A=lat_last; //pointer to the second part of latitude 
	point->lon_first_A=lon_first; //pointer to first part of longitude day 
...
//SNR channel 4
	...
	//pointers for members of the struct 
...
//Print function to output to the console the information reuqired 
void printFinal(struct Data point)
{

/* to make sure that no junk is printed and the values come out clean 
of zeros if there is  no data extracted for following variables*/
//Takes care of all the empty values right when the gps parser starts. 
     bool present=false;
	 
	//if the couse is passed and is there value it will proceed as true 
	 ...
	 //get the fix if is 2d , 3d or no fix depending if the fix is read form file. comes true 
	 string fix;
	...
	if(point.UTC.empty()==true)//makes sure that no junk is displayed
	{
		point.UTC_time=0;
	}	 
	if(point.DATE.empty()==true)  //...same
	{
		point.date_day="00";
		point.date_month="00";  
		point.year=0;
	}
	if(point.latitude.empty() ==true )   //...same 
	{
		point.lat_first_A=0;
		point.lat_last_A=0;
	}
	if(point.longitude.empty() ==true ) //...same 
	{
		point.lon_first_A=0;
		point.lon_last_A=0;
	}
	if(point.sat_used.empty()==true){
	
		point.satelites_A=0;
	}
	if(point.MSL.empty() ==true) //...same 
	{
		point.MSL_Altitude_A=0;
	}
	if(point.speed_C.empty() ==true) //...same 
	{
		point.speed_R=-1;
	}
	if(point.course_C.empty() ==true) //...same 
	{
		point.course_R=-1;
	}
	if(point.PDOP_A.empty()==true) //..same
	{
		point.PDOP=-1;
	}
		if(point.HDOP_A.empty()==true) //...same
	{
		point.HDOP=-1;
	}
		if(point.VDOP_A.empty()==true) //..same
	{
		point.VDOP=-1;
	}
		...
		cout<<"Altitude: "<<setprecision(2)<<point.MSL_Altitude_A<<"M"<<endl;
